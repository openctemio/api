...
--
-- Usage: psql -U postgres -d openctem -f scripts/setup_production_db_user.sql
--
-- After running this script, update your DATABASE_URL:
--   DATABASE_URL=postgres://openctem_app:YOUR_PASSWORD@host:5432/openctem
-- =============================================================================
...
-- =============================================================================
\echo '>>> Step 1: Creating openctem_app user...'

DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'openctem_app') THEN
        -- IMPORTANT: Change this password in production!
        CREATE ROLE openctem_app LOGIN PASSWORD 'change-me-in-production';
        RAISE NOTICE 'Created openctem_app role';
    ELSE
        RAISE NOTICE 'openctem_app role already exists';
    END IF;
END
$$;
...
WHERE object_schema = 'public' AND grantee = 'openctem_app';
...
\echo ''
\echo 'Next steps:'
\echo '  1. Change the password for openctem_app in production:'
\echo '     ALTER ROLE openctem_app PASSWORD ''your-secure-password'';'
\echo ''
\echo '  2. Update your DATABASE_URL:'
\echo '     DATABASE_URL=postgres://openctem_app:YOUR_PASSWORD@host:5432/openctem'
\echo ''
\echo '  3. Verify RLS is working by connecting as openctem_app and running:'
