...
-- Note: not using CONCURRENTLY — golang-migrate runs each file in a transaction,
...
CREATE INDEX IF NOT EXISTS idx_assets_tenant_type_crit_status
    ON assets(tenant_id, asset_type, criticality, status);

-- Compound index for discovery source queries per tenant
CREATE INDEX IF NOT EXISTS idx_assets_tenant_discovery_source
