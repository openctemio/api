...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scope_rules_tenant_active
    ON group_asset_scope_rules (tenant_id, is_active)
    WHERE is_active = true;

-- Scope rules: list active rules per group (used by ReconcileGroup)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scope_rules_group_tenant_active
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_asset_owners_source
