...
func (s *OutboxService) EnqueueNotification(ctx context.Context, params EnqueueNotificationParams) error {
	outbox := outbox.NewOutbox(outbox.OutboxParams{
		...
		Title:         params.Title,
		Body:          params.Body,
		Severity:      outbox.Severity(params.Severity),
		URL:           params.URL,
...
func (s *OutboxService) processOutboxEntry(ctx context.Context, entry *outbox.Outbox) error {
...
func (s *OutboxService) getNotificationIntegrationsForTenant(ctx context.Context, tenantID shared.ID) ([]*integration.IntegrationWithNotification, error) {
...
func (s *OutboxService) shouldSendToIntegration(intg *integration.IntegrationWithNotification, entry *outbox.Outbox) bool {
...
func (s *OutboxService) sendToIntegration(ctx context.Context, intg *integration.IntegrationWithNotification, entry *outbox.Outbox) outbox.SendResult {
	ext := intg.Notification
	sentAt := time.Now()
...
func (s *OutboxService) configureClientFromCredentials(
	config *notificationclient.Config,
