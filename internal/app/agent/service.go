...
func (s *AgentService) CreateAgent(ctx context.Context, input CreateAgentInput) (*CreateAgentOutput, error) {
	...
	agentType := agentdom.AgentType(input.Type)
	executionMode := agentdom.ExecutionMode(input.ExecutionMode)
	if executionMode == "" {
		executionMode = agentType.DefaultExecutionMode()
	}

	a, err := agentdom.NewAgent(tenantID, input.Name, agentType, input.Description, input.Capabilities, input.Tools, executionMode)
	if err != nil {
		return nil, err
	}
...
func (s *AgentService) ListAgents(ctx context.Context, input ListAgentsInput) (pagination.Result[*agentdom.Agent], error) {
	tenantID, err := shared.IDFromString(input.TenantID)
	if err != nil {
		return pagination.Result[*agentdom.Agent]{}, fmt.Errorf("%w: invalid tenant id", shared.ErrValidation)
	}

	filter := agentdom.Filter{
		TenantID:     &tenantID,
	...
	if input.Type != "" {
		t := agentdom.AgentType(input.Type)
		filter.Type = &t
	}

	if input.Status != "" {
		st := agentdom.AgentStatus(input.Status)
		filter.Status = &st
	}

	if input.ExecutionMode != "" {
		em := agentdom.ExecutionMode(input.ExecutionMode)
		filter.ExecutionMode = &em
	}

	if input.Health != "" {
		h := agentdom.AgentHealth(input.Health)
		filter.Health = &h
	}
...
func (s *AgentService) UpdateAgent(ctx context.Context, input UpdateAgentInput) (*agentdom.Agent, error) {
	...
	if input.Status != "" {
		...
		a.SetStatus(agentdom.AgentStatus(input.Status), "")
