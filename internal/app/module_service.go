...
func (s *ModuleService) GetTenantEnabledModules(ctx context.Context, tenantID string) (*GetTenantEnabledModulesOutput, error) {
	...
	enabledModules := make([]*module.Module, 0, len(topLevel))
	...
	subModules := make(map[string][]*module.Module, len(subModulesByParent))
	for parentID, subs := range subModulesByParent {
		if enabledSet[parentID] {
			enabledSubs := make([]*module.Module, 0, len(subs))
...
func (s *ModuleService) buildTenantModuleConfig(ctx context.Context, tenantID string, allModules []*module.Module) (*TenantModuleConfigOutput, error) {
	// Split into top-level and sub-modules from the same query result
	topLevel, subModulesByParent := splitModules(allModules)
...
func (s *ModuleService) UpdateTenantModules(ctx context.Context, tenantID string, updates []module.TenantModuleUpdate, actx AuditContext) (*TenantModuleConfigOutput, error) {
	...
	moduleMap := make(map[string]*module.Module, len(allModules))
	...
	for _, u := range updates {
		...
		if !m.IsActive() {
			return nil, fmt.Errorf("%w: module '%s' is not available", module.ErrModuleNotAvailable, u.ModuleID)
		}

		if !u.IsEnabled && m.IsCore() {
			return nil, fmt.Errorf("%w: '%s' is a core module and cannot be disabled", module.ErrCoreModuleCannotBeDisabled, m.Name())
		}
...
func (s *ModuleService) logModuleAudit(ctx context.Context, actx AuditContext, tenantID string, enabled, disabled []string) {
	...
	event := NewSuccessEvent(audit.ActionTenantModulesUpdated, audit.ResourceTypeTenant, tenantID).
		WithMessage(msg).
...
func splitModules(allModules []*module.Module) ([]*module.Module, map[string][]*module.Module) {
	topLevel := make([]*module.Module, 0, len(allModules))
	subByParent := make(map[string][]*module.Module)
