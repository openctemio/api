...
type PermissionService struct {
	...
	resolver          *accesscontroldom.PermissionResolver
...
func NewPermissionService(
	...
	s := &PermissionService{
		...
		resolver:          accesscontroldom.NewPermissionResolver(),
...
func (s *PermissionService) logAudit(ctx context.Context, actx auditapp.AuditContext, event auditapp.AuditEvent) {
...
func (s *PermissionService) CreatePermissionSet(ctx context.Context, input CreatePermissionSetInput, actx auditapp.AuditContext) (*permissionsetdom.PermissionSet, error) {
	...
	setType := permissionsetdom.SetType(input.SetType)
	if !setType.IsValid() {
	...
	if (setType == permissionsetdom.SetTypeExtended || setType == permissionsetdom.SetTypeCloned) && parentSetID == nil {
	...
	var ps *permissionsetdom.PermissionSet
	switch setType {
	case permissionsetdom.SetTypeExtended:
		ps, err = permissionsetdom.NewExtendedPermissionSet(tenantID, input.Name, input.Slug, input.Description, *parentSetID)
	case permissionsetdom.SetTypeCloned:
		...
		ps, err = permissionsetdom.NewClonedPermissionSet(tenantID, input.Name, input.Slug, input.Description, *parentSetID, sourceVersion)
	default:
		// Custom type
		ps, err = permissionsetdom.NewPermissionSet(tenantID, input.Name, input.Slug, input.Description)
	}
	...
	if len(input.Permissions) > 0 {
		for _, permID := range input.Permissions {
			item, err := permissionsetdom.NewItem(ps.ID(), permID, permissionsetdom.ModificationAdd)
			if err != nil {
	...
	event := auditapp.NewSuccessEvent(audit.ActionPermissionSetCreated, audit.ResourceTypePermissionSet, ps.ID().String()).
...
func (s *PermissionService) UpdatePermissionSet(ctx context.Context, permissionSetID string, input UpdatePermissionSetInput, actx auditapp.AuditContext) (*permissionsetdom.PermissionSet, error) {
	...
	event := auditapp.NewSuccessEvent(audit.ActionPermissionSetUpdated, audit.ResourceTypePermissionSet, permissionSetID).
		WithResourceName(ps.Name()).
...
func (s *PermissionService) DeletePermissionSet(ctx context.Context, permissionSetID string, actx auditapp.AuditContext) error {
	...
	event := auditapp.NewSuccessEvent(audit.ActionPermissionSetDeleted, audit.ResourceTypePermissionSet, permissionSetID).
...
func (s *PermissionService) ListPermissionSets(ctx context.Context, input ListPermissionSetsInput) (*ListPermissionSetsOutput, error) {
	...
	filter := permissionsetdom.ListFilter{
		TenantID:      &tenantID,
	...
	if input.SetType != nil {
		st := permissionsetdom.SetType(*input.SetType)
		filter.SetTypes = []permissionsetdom.SetType{st}
	}
...
func (s *PermissionService) AddPermissionToSet(ctx context.Context, input AddPermissionToSetInput, actx auditapp.AuditContext) error {
	...
	modType := permissionsetdom.ModificationAdd
	if input.ModificationType != "" {
		modType = permissionsetdom.ModificationType(input.ModificationType)
	}

	item, err := permissionsetdom.NewItem(permSetID, input.PermissionID, modType)
	if err != nil {
		return err
	}
	...
	event := auditapp.NewSuccessEvent(audit.ActionPermissionSetUpdated, audit.ResourceTypePermissionSet, input.PermissionSetID).
...
func (s *PermissionService) ResolveGroupPermissions(ctx context.Context, groupID string) ([]permissiondom.Permission, error) {
	...
	permissionSets := make([]*permissionsetdom.PermissionSetWithItems, 0, len(permSetIDs))
	...
	var customPermissions []*accesscontroldom.GroupPermission
...
func (s *PermissionService) HasPermission(ctx context.Context, tenantID string, userID shared.ID, perm permissiondom.Permission) (bool, error) {
	...
	return permissiondom.Contains(perms, perm), nil
}
...
func (s *PermissionService) HasAnyPermission(ctx context.Context, tenantID string, userID shared.ID, perms ...permissiondom.Permission) (bool, error) {
	...
	return permissiondom.ContainsAny(userPerms, perms...), nil
}
...
func (s *PermissionService) HasAllPermissions(ctx context.Context, tenantID string, userID shared.ID, perms ...permissiondom.Permission) (bool, error) {
	...
	return permissiondom.ContainsAll(userPerms, perms...), nil
}
...
func (s *PermissionService) CreateGroupPermission(ctx context.Context, input CreateGroupPermissionInput, createdBy shared.ID, actx auditapp.AuditContext) (*accesscontroldom.GroupPermission, error) {
	if s.accessControlRepo == nil {
	...
	effect := accesscontroldom.PermissionEffect(input.Effect)
	if !effect.IsValid() {
	...
	gp, err := accesscontroldom.NewGroupPermission(groupID, input.PermissionID, effect, &createdBy)
	if err != nil {
		return nil, err
	}
	...
	event := auditapp.NewSuccessEvent(audit.ActionPermissionGranted, audit.ResourceTypeGroup, input.GroupID).
...
func (s *PermissionService) DeleteGroupPermission(ctx context.Context, groupID, permissionID string, actx auditapp.AuditContext) error {
	...
	// Log audit event
	actx.TenantID = g.TenantID().String()
	event := auditapp.NewSuccessEvent(audit.ActionPermissionRevoked, audit.ResourceTypeGroup, groupID).
