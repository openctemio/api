...
// classify to the highest priority class. Each one triggers: Jira
// ticket, outbox notification, priority-queue jump, assignment-rule
// sweep. The tenant's inbox melts; operators start treating the top
// class as noise; a real one arriving in the middle of the burst is
// lost.
//
// Solution: per-tenant rolling-window cap on top-class emissions.
// Once the cap is hit, additional classifications at that class are:
//   - still RECORDED (class unchanged, reason kept, audit-logged as
...
// the accurate count on the dashboard; only the operational firehose
// is dampened.

// PriorityFloodConfig tunes the guard. Zero values take documented
// defaults.
...
// PriorityFloodGuard is per-process in-memory. A future iteration
// moves it to Redis for cross-replica dedup; as long as there is ONE
// background worker doing classification (the sweep controller),
// in-memory is sufficient.
type PriorityFloodGuard struct {
	cfg   PriorityFloodConfig
	mu    sync.Mutex
...
func NewPriorityFloodGuard(cfg PriorityFloodConfig) *PriorityFloodGuard {
	if cfg.ProtectedClass == "" {
		cfg.ProtectedClass = vulnerability.PriorityP0
	}
	if cfg.MaxPerHour <= 0 {
		cfg.MaxPerHour = 50
	}
	...
	return &PriorityFloodGuard{cfg: cfg, usage: make(map[shared.ID][]time.Time)}
}

// ErrPriorityFloodSuppressed is returned by ShouldFanOut when the
// tenant has already consumed its rolling budget. Classification is
// NOT reverted; only downstream side effects are skipped.
...
// outbox, fast-lane queue) should run for a newly-classified finding
// at the guard's protected class.
//
// Returns (true, nil) under budget — record AND fire.
// Returns (false, ErrPriorityFloodSuppressed) over budget — record only.
//
// A classification that is NOT the protected class short-circuits
// immediately and the budget is not charged.
func (g *PriorityFloodGuard) ShouldFanOut(
	ctx context.Context,
	tenantID shared.ID,
	class vulnerability.PriorityClass,
) (bool, error) {
	if class != g.cfg.ProtectedClass {
		return true, nil
	}
	...
	if len(kept) >= g.cfg.MaxPerHour {
		return false, ErrPriorityFloodSuppressed
	}
...
func (g *PriorityFloodGuard) pruneLocked(tenantID shared.ID, at time.Time) []time.Time {
