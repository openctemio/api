...
func TestBuildPriorityContext_OpenThreatPathPromotesToP0(t *testing.T) {
	...
	ctx := svc.buildPriorityContext(newTestFinding(t), a, nil, threatened)
...
func TestBuildPriorityContext_ThreatPathRaisesRelativeToOffPath(t *testing.T) {
	...
	onCtx := svc.buildPriorityContext(newTestFinding(t), onPath, nil, threatened)
	offCtx := svc.buildPriorityContext(newTestFinding(t), offPath, nil, threatened)
...
func TestBuildPriorityContext_ThreatPathRaisesMediumFinding(t *testing.T) {
	...
	a := newTestAsset(t, asset.ExposurePrivate)
	off := vulnerability.ClassifyPriority(svc.buildPriorityContext(mkMedium(), a, nil, nil))
	on := vulnerability.ClassifyPriority(svc.buildPriorityContext(mkMedium(), a, nil, map[string]bool{a.ID().String(): true}))
...
func TestBuildPriorityContext_NoThreatModelNoChange(t *testing.T) {
	...
	base := vulnerability.ClassifyPriority(svc.buildPriorityContext(newTestFinding(t), a, nil, nil))
	empty := vulnerability.ClassifyPriority(svc.buildPriorityContext(newTestFinding(t), a, nil, map[string]bool{}))
...
func TestBuildPriorityContext_ThreatSetDoesNotLeak(t *testing.T) {
	svc := newReachabilitySvc()
	a := newTestAsset(t, asset.ExposurePrivate)
	ctx := svc.buildPriorityContext(newTestFinding(t), a, nil, map[string]bool{"some-other-asset": true})
	if ctx.OnOpenThreatPath {
